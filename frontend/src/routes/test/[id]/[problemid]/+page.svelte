<script lang="ts">
	...
	const getStarterCode = () => {
		...
		const args = problem.input_variables.map((input: any) => input.name).join(', ');
		return `#Do not change the function definition of the below function\ndef ${functionName}(${args}):`;
	};
	onMount(async () => {
		const res = await API.get(`/test/${$page.params.id}`);
		problem = res.data.problems.find((problem: any) => problem._id === $page.params.problemid);
		code = getStarterCode();
	});
		...
		<Pane size={40} minSize={40} maxSize={60} class="problem">
			<div class="block">
				...
				<div class="titleMarks">
					...
					{#each problem.test_cases as _, index}
						<IoBlock index={index + 1} testCase={problem.test_cases[index]} />
					{/each}
								...
								<div class="message">
									...
									<div class="userdetails">
										...
										<p class="messagecontent">
											...
											fuga magni dignissimos accusantium itaque, tempora repellat laborum reiciendis
