...
export const userEmailVerificationSendController = async (req: Request, res: Response) => {
    try {
        ...
        if(redisData) return res.status(400).json({ type: "ERROR", message: "Email already sent", time: JSON.parse(redisData).time })
        ...
        await redisClient.v4.set(`email_verification_code:${email}`, JSON.stringify({ code: code, time: 10 * 60 }), 'EX', 10 * 60)
